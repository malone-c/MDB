...
neighbours = list(Gwydir = c("Namoi"),
									Lachlan = c("Murrumbidgee", "Macq.Castle", "Lower.D"),
									Lower.D = c("Lachlan", "Murrumbidgee", "Lower.M.NSW", "Lower.M.Vic"),
									Macq.Castle = c("Namoi", "Lachlan"),
									Upper.M.NSW = c("Murrumbidgee"),
									Lower.M.NSW = c("Lower.D", "Lower.M.Vic", "Lower.M.SA"),
									Murrumbidgee = c("Lower.D", "Lachlan", "Upper.M.NSW", "Upper.M.Vic"),
									Namoi = c("Gwydir", "Macq.Castle"),
									Murray.SA = c("Lower.D"),
									Goulburn = c("Loddon", "Murrumbidgee"),
									Upper.M.Vic = c("Murrumbidgee"),
									Lower.M.Vic = c("Lower.M.SA", "Lower.M.NSW", "Lower.D", "Loddon")
									)

require(plyr)


regions[2] %in% unlist(neighbours[3])

data.regions = murray[16:29, ]

zero.matrix = matrix(0, nrow = N * t, ncol = N * t)

gamma = 2
mdb = factor(read.csv('data/murray.csv')$region)
levels(mdb) = regions

for (i in seq(1, N*t, N)) {
	for (j in seq(1, N*t, N)) {
		if (mdb[i] %in% unlist[(mdb[j])]) {
			
		}
	}
}

mdb[100] %in% unlist(neighbours[mdb[200]])

# Note: Make sure to include regions in the model as well.
# This is so that we can capture part of the relationship
# between different crops in the same region. Maybe fit
# regression slopes for each region or each individual.

GY.single = abs(outer((1:t), (1:t), "-")) +1
